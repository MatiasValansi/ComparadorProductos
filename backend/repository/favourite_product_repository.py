...
class FavoriteProductRepository:
    ...
    async def get_one_fav_by_id(favorite_id:int):
        fav_found = await favourites_collection.find_one({"_id": favorite_id})
        return fav_found
    ...
    async def update_fav(favorite_id:int, fav_to_update:FavouriteProduct):
    ...
    async def delete_fav(favorite_id:int):
